...
|---|---|---|
| `act/workflows/host-tests.yml` | the host-side C/C++ unit suites (nostr_store 7, relay-pipeline 13, tracker tollgate-proto 106, payack-parser 14, tollgate ~350, ehash-relay 65, stratorelay 11) | same commands as `.github/workflows/ci-host-tests.yml`; one harness fix serves both engines |
...
Local reproduction of the ngit lane: run the same command blocks from the
