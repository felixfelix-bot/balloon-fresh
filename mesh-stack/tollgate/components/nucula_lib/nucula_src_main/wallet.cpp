...
static const char* WALLET_BASE = "/spiffs/wallet";
...
static void wallet_dir(char* buf, size_t sz, int slot)
{
    snprintf(buf, sz, "%s/%d", WALLET_BASE, slot);
}

static void wallet_file(char* buf, size_t sz, int slot, const char* name)
{
    snprintf(buf, sz, "%s/%d/%s", WALLET_BASE, slot, name);
}

static bool ensure_wallet_dir(int slot)
{
    struct stat st;
    if (stat(WALLET_BASE, &st) != 0) mkdir(WALLET_BASE, 0775);
    char dir[64];
    wallet_dir(dir, sizeof(dir), slot);
    if (stat(dir, &st) != 0) {
        if (mkdir(dir, 0775) != 0) {
...
static bool write_file_atomic(const char* path, const std::string& data)
{
    ...
    if (written != data.size()) {
        ESP_LOGE(TAG, "write %s incomplete (%zu/%zu)", tmp, written, data.size());
        unlink(tmp);
        return false;
    }
...
// -------------------------------------------------------------------------
// SPIFFS file persistence (with NVS migration fallback)
// -------------------------------------------------------------------------

bool Wallet::save_mint_url()
{
    if (!ensure_wallet_dir(nvs_slot_)) return false;
    char path[80];
    wallet_file(path, sizeof(path), nvs_slot_, "url.txt");
    return write_file_atomic(path, mint_url_);
}

std::string Wallet::load_mint_url_for_slot(int slot)
{
    char path[80];
    wallet_file(path, sizeof(path), slot, "url.txt");
    ...
    if (!url.empty()) {
        ensure_wallet_dir(slot);
        write_file_atomic(path, url);
        nvs_handle_t wh;
        if (nvs_open(NVS_NS, NVS_READWRITE, &wh) == ESP_OK) {
            nvs_erase_key(wh, key);
            nvs_commit(wh);
            nvs_close(wh);
        }
    }
...
static const char* COUNTERS_PATH = "/spiffs/wallet/counters.json";

uint32_t Wallet::load_counter(const std::string& keyset_id)
{
    std::string data = read_file(COUNTERS_PATH);
    if (data.empty()) {
        nvs_handle_t handle;
    ...
    if (!root) return 0;
    cJSON* item = cJSON_GetObjectItemCaseSensitive(root, keyset_id.c_str());
    uint32_t val = 0;
    if (item && cJSON_IsNumber(item)) val = (uint32_t)item->valuedouble;
    cJSON_Delete(root);
    return val;
}
...
bool Wallet::save_counter(const std::string& keyset_id, uint32_t counter)
{
    ...
    cJSON_AddNumberToObject(root, keyset_id.c_str(), (double)counter);
    ...
    return write_file_atomic(COUNTERS_PATH, out);
}
...
bool Wallet::save_proofs()
{
    ...
    if (!ensure_wallet_dir(nvs_slot_)) return false;
    char path[80];
    wallet_file(path, sizeof(path), nvs_slot_, "proofs.json");
    if (!write_file_atomic(path, blob)) {
        ESP_LOGE(TAG, "save_proofs failed: write error");
        return false;
    }
...
bool Wallet::load_proofs()
{
    ...
    wallet_file(path, sizeof(path), nvs_slot_, "proofs.json");
    std::string data = read_file(path);
    bool from_file = !data.empty();

    if (!from_file) {
        nvs_handle_t handle;
        esp_err_t err = nvs_open(NVS_NS, NVS_READONLY, &handle);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "[%d] load_proofs: nvs_open failed: %s", nvs_slot_, esp_err_to_name(err));
            return false;
        }
        char key[16];
        slot_key(key, sizeof(key), "proofs", nvs_slot_);
        ...
        if (err != ESP_OK || required == 0) {
            ESP_LOGW(TAG, "[%d] load_proofs: no blob for key %s (err=%s, size=%d)",
                     nvs_slot_, key, esp_err_to_name(err), (int)required);
            nvs_close(handle);
            return false;
        }
        data.resize(required);
        err = nvs_get_blob(handle, key, data.data(), &required);
        nvs_close(handle);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "[%d] load_proofs: nvs_get_blob failed: %s", nvs_slot_, esp_err_to_name(err));
            return false;
        }
    }

    std::vector<Proof> loaded;
    if (!proofs_from_json(data.c_str(), loaded)) {
        ESP_LOGW(TAG, "[%d] load_proofs: JSON parse failed for %d bytes (data corrupted, clearing)", nvs_slot_, (int)data.size());
        if (from_file) {
            remove(path);
        }
        return false;
    ...
    ESP_LOGI(TAG, "[%d] loaded %d proofs from %s (%d bytes)",
             nvs_slot_, (int)proofs_.size(), from_file ? "file" : "NVS", (int)data.size());

    if (!from_file && !proofs_.empty()) {
        ensure_wallet_dir(nvs_slot_);
...
bool Wallet::save_keysets()
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        cJSON* item = cJSON_Parse(blob.c_str());
    ...
    if (!ensure_wallet_dir(nvs_slot_)) return false;
    char path[80];
    wallet_file(path, sizeof(path), nvs_slot_, "keysets.json");
    if (!write_file_atomic(path, data)) {
        ESP_LOGE(TAG, "save_keysets write failed");
        return false;
    }
...
bool Wallet::load_keysets_nvs()
{
    ...
    if (!data.empty()) {
        ...
        if (json && cJSON_IsArray(json)) {
            ...
            for (int i = 0; i < n && i < MAX_KEYSETS; i++) {
                cJSON* item = cJSON_GetArrayItem(json, i);
                Keyset ks{};
                if (deserialize(cJSON_PrintUnformatted(item), ks))
                    loaded.push_back(std::move(ks));
            }
            cJSON_Delete(json);
            if (!loaded.empty()) {
                keysets_ = std::move(loaded);
                ESP_LOGI(TAG, "[%d] loaded %d keysets from file",
    ...
    // NVS migration: try individual-entry format
    nvs_handle_t handle;
    esp_err_t err = nvs_open(NVS_NS, NVS_READONLY, &handle);
    ...
    if (err == ESP_OK && count > 0) {
        for (int i = 0; i < count && i < MAX_KEYSETS; i++) {
            ...
            size_t required = 0;
            if (nvs_get_blob(handle, ks_key, nullptr, &required) != ESP_OK || required == 0)
                continue;
    ...
    if (loaded.empty()) {
        ...
        slot_key(key, sizeof(key), "keys", nvs_slot_);
        size_t required = 0;
        if (nvs_get_blob(handle, key, nullptr, &required) == ESP_OK && required > 0) {
            std::string blob(required, '\0');
            if (nvs_get_blob(handle, key, blob.data(), &required) == ESP_OK)
                keysets_from_json(blob.c_str(), loaded);
        }
    }
    nvs_close(handle);
    ...
    if (!loaded.empty()) {
        keysets_ = std::move(loaded);
        ESP_LOGI(TAG, "[%d] loaded %d keysets from NVS (migrated)",
                 nvs_slot_, (int)keysets_.size());
        save_keysets();
        return true;
    }
...
const Keyset* Wallet::keyset_for_id(const std::string& id) const
{
    for (const auto& k : keysets_) {
        if (k.id == id)
            return &k;
        if (k.id.size() > id.size() && k.id.substr(0, id.size()) == id)
            return &k;
        if (id.size() > k.id.size() && id.substr(0, k.id.size()) == k.id)
            return &k;
    }
    return nullptr;
}
                  ...
                  const std::string& unit)
{
    ...
    const Keyset* ks = active_keyset(unit);
    if (!ks) {
        ESP_LOGE(TAG, "swap: no active keyset (unit=%s)", unit.c_str());
        return false;
    }
    ...
    for (auto& p : stripped) {
        const Keyset* pks = keyset_for_id(p.id);
        if (pks && pks->id.size() > p.id.size())
            p.id = pks->id;
    }
...
bool Wallet::receive(const Token& token, std::vector<Proof>& proofs_out)
{
    ...
    std::string unit = token.unit.empty() ? "sat" : token.unit;

    std::vector<Proof> new_proofs, change;
    if (!swap(inputs, -1, new_proofs, change, unit))
        return false;
