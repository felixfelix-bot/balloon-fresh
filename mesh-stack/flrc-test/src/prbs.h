...
static inline void prbs15_fill(uint8_t *buf, size_t len, uint32_t seed) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        for (int b = 0; b < 8; b++) {
            ...
            byte = (byte << 1) | (newbit & 1);
        }
        buf[i] = byte;
    }
}

static inline uint16_t prbs15_verify(const uint8_t *buf, size_t len, uint32_t seed, uint16_t *out_bytes_bad) {
    uint16_t state = (uint16_t)(seed ^ 0x5A5A) | 1;
