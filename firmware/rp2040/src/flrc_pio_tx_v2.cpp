/*
 * flrc_pio_tx_v2.cpp — Hybrid Arduino+PIO SPI TX (CDC-safe)
 * ============================================================================
 *
 * FIX for v1 CDC crash: uses Arduino SPI for radio init (CDC-safe),
...
static void runTransmit() {
    ...
    for (int i = 0; i < TX_PKT_COUNT; i++) {
        ...
        if (i < 5) {
            dualPrintf("PKT %d: irqPin=%d spin=%lu", i, irqFired ? 1 : 0,
        ...
        if ((i + 1) % 200 == 0) {
            dualPrintf("TX %d/%d (done=%lu to=%lu)", i + 1, TX_PKT_COUNT,
...
void setup() {
    ...
    if (radioReady) {
        ...
        spiRf.end();
        Serial1.println("Arduino SPI released. Starting PIO+DMA...");
        ...
        Serial1.println("PIO+DMA SPI init OK");
        Serial.println("PIO INIT OK — starting TX");
