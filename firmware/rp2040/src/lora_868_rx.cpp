/*
 * lora_868_rx.cpp — 868 MHz LoRa Autonomous RX for outdoor range testing
 *
 * Based on lora_range_rx.cpp (proven LoRa RX at 2.4 GHz).
 * Modified for 868 MHz sub-GHz (European ISM band) on LR2021 LF path.
 *
 * THREE CRITICAL CHANGES from 2.4 GHz firmware:
 *   1. RX_FREQ_MHZ = 868.0f (was 2440.0f)
 *   2. SET_RX_PATH = 0x00 (LF=sub-GHz, was 0x01=HF=2.4 GHz)
 *   3. CALIB_FRONT_END feFreq: no 0x8000 bit (HF bit removed for LF)
 *
 * Behavior:
 * - On boot: init radio, auto-start RX listen
 * - RX loop: poll IRQ pin → readFifo → re-arm → readLoraRssi+SNR
 * - DEADBEEF end marker detection (reads TX total packet count)
 * - Time-based window: LISTEN_MS=30000, SILENCE_MS=5000
 * - LED blinks on each packet received
 * - RSSI/SNR via GET_LORA_PACKET_STATUS (0x022A): buf[2]=RSSI, buf[3]=SNR
 *
 * LoRa config: SF7, BW 812 kHz, CR 4/5 (LORA_CR=1)
 *              127-byte packets
 *
 * Output: LORA_868_RX_RESULT,rx=N,unique=N,lost=N,...,freq=868.0,...
 *
...
static bool rawInitRadio() {
    ...
    if ((st >> 4) == 0x05) {
        dualPrintln("RADIO_INIT_OK (LoRa RX 868 MHz LF)");
        return true;
...
struct RxStats {
    ...
    uint32_t lastSeq;
    uint32_t maxSeq;
    uint32_t totalSentByTx; // from DEADBEEF marker
    uint32_t startMs;
    uint32_t elapsedMs;
    int32_t  rssiSum;
    int16_t  rssiMin;
...
static void runReceive() {
    ...
    dualPrintf("LORA_868_RX_WINDOW %lu START uptime=%lums listen=%dms sf=%d bw=%d freq=%.1f",
               (unsigned long)windowNum, (unsigned long)stats.startMs,
               RX_LISTEN_MS, cfgSf, bwKhz, cfgFreq);
    ...
    while (true) {
        ...
        // Read LoRa RSSI/SNR via GET_LORA_PACKET_STATUS (0x022A)
        ...
        // DEADBEEF end marker — reads total packet count sent by TX
    ...
    dualPrintf("LORA_868_RX_RESULT,window=%lu,rx=%lu,cum_rx=%lu,unique=%lu,lost=%lu,total=%lu,per=%.2f,elapsed_ms=%lu,throughput_kbps=%.1f,rssi_avg=%.1f,rssi_min=%d,snr_avg=%.1f,sf=%d,bw=%d,cr=4/%d,pktSize=%d,freq=%.1f,uptime_ms=%lu",
...
void setup() {
    ...
    Serial.println("BOOT LORA 868 RX RANGE");
    ...
    dualPrintln();
    dualPrintln("=============================================");
    dualPrintln("=== RP2040 LORA 868 MHz RX (AUTONOMOUS) ===");
    dualPrintln("=============================================");
    dualPrintf("Config: freq=%.1f MHz sf=%d bw=%d cr=4/%d pktSize=%d listen=%dms",
    ...
    if (radioReady) {
        ...
        dualPrintln("AUTO RX LISTENING (LoRa 868 MHz)");
    } else {
        ...
        if (radioReady) {
            digitalWrite(PIN_LED_ALT, HIGH);
            dualPrintln("AUTO RX LISTENING (2nd init, LoRa 868 MHz)");
        } else {
