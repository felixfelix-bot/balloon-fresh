/*
 * flrc_bitrate_tx.cpp — RP2040 FLRC TX with COMPILE-TIME bitrate selection
 *
 * Based on flrc_raw_tx.cpp (verified: 1753 kbps TX, 0% PER at 2600 kbps).
 *
 * Bitrate is selected at COMPILE TIME via -DFLRC_BITRATE=<kbps>.
 * No runtime serial bitrate commands (USB CDC dies during TX loops).
 *
 * Build:
 *   pio run -e rp2040-flrc-tx-2600   (default)
 *   pio run -e rp2040-flrc-tx-1300
 *   pio run -e rp2040-flrc-tx-650
 *   pio run -e rp2040-flrc-tx-325
 *
...
// Matches LR2021Raw.h bitrateToCode() and flrc_range_rx.cpp bitrateToBrBw()
static constexpr uint8_t bitrateToBrBw(uint16_t kbps) {
    switch (kbps) {
...
static bool rawInitRadio() {
    ...
    // brBw = compile-time FLRC_BITRATE → code mapping
    // CR=0x25 (uncoded, BT=0.5)
    {
        uint8_t brBw = bitrateToBrBw(FLRC_BITRATE);
    ...
    dualPrintf("INIT Status=0x%02X IRQ=0x%08lX brBw=0x%02X bitrate=%d",
               st, (unsigned long)irq, bitrateToBrBw(FLRC_BITRATE), FLRC_BITRATE);
...
static void runTransmit() {
    ...
    dualPrintf("TX_START count=%d pktSize=%d bitrate=%d", TX_PKT_COUNT, FLRC_PKT_SIZE, FLRC_BITRATE);
    delay(10);
    ...
    dualPrintf("  Bitrate:     %d kbps", FLRC_BITRATE);
    dualPrintf("  Elapsed:     %lu ms", (unsigned long)elapsed);
    dualPrintf("  TX THROUGHPUT: %.1f kbps", tput);
    dualPrintln("=============================================");
    dualPrintf("RESULT_TX,sent=%d,elapsed_ms=%lu,throughput_kbps=%.1f,bitrate=%d",
               TX_PKT_COUNT, (unsigned long)elapsed, tput, FLRC_BITRATE);
}
...
void setup() {
    ...
    Serial1.println();
    Serial1.println("=== RP2040 FLRC BITRATE TX ===");
    Serial1.print("Compile-time bitrate: ");
    Serial1.print(FLRC_BITRATE);
    Serial1.println(" kbps");
    Serial1.println("Pure Arduino SPI + IRQ poll + 20MHz");
