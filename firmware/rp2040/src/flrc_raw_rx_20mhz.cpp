/*
 * flrc_raw_rx_20mhz.cpp — RP2040 FLRC RX with RAW SPI init (no RadioLib)
 * 20MHz test variant — SPI clock bumped from 16MHz to 20MHz (must match TX).
 *
...
void setup() {
    ...
    dualPrintln();
    dualPrintln("=== RP2040 FLRC RAW RX 20MHz ===");
    dualPrintln("Raw SPI init (no RadioLib) — 20MHz SPI clock");
