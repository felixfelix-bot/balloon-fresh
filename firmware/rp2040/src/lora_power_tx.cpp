/*
 * lora_power_tx.cpp — LoRa TX with compile-time TX power selection
 *
 * Uses raw 2-byte opcode SPI (ADR-020). No RadioLib.
 * LoRa init sequence verified against RadioLib v7.6.0 source.
 *
 * Power is selected at compile time via -DTX_POWER_DBM=<dBm>.
 * Build one firmware per power level for the power sweep:
 *   pio run -e rp2040-lora-tx-sf7-pwr0    (0 dBm)
 *   pio run -e rp2040-lora-tx-sf7-pwr3    (3 dBm)
 *   pio run -e rp2040-lora-tx-sf7-pwr6    (6 dBm)
 *   pio run -e rp2040-lora-tx-sf7-pwr12   (12 dBm)
 *
 * Behavior:
...
void setup() {
    ...
    int bwKhz = bwCodeToKhz(cfgBwCode);
    dualPrintln();
    dualPrintln("=== RP2040 LORA POWER TX ===");
    dualPrintf("Config: freq=%.1f sf=%d bw=%d cr=4/%d pktSize=%d power=%.1f count=%lu (TX_POWER_DBM=%.1f)",
               cfgFreq, cfgSf, bwKhz, (cfgCr + 4), cfgPktSize, cfgPower,
               (unsigned long)cfgCount, (float)TX_POWER_DBM);
