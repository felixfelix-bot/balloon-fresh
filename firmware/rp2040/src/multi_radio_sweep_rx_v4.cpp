...
typedef struct {
    ...
    uint16_t pktSize;    // V4: payload size (32/64/128/255)
} Phase;
...
// (totalCycleSec declared earlier — V4 forward declaration)
...
static void rxPacketPoll(int phaseIdx) {
    ...
    // ─── V4: App-layer CRC-16 over FULL variable payload ────────────
    // CRC-16 (CCITT 0x1021) covers bytes gpsOff+0 through gpsOff+crcLen-1
...
void setup() {
    ...
    // V4: init sync offset cache + build interleave table
