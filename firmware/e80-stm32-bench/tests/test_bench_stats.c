...
static void test_per(void)
{
    ...
    /* only first seen, seq span 1, ok 1 -> 0; ok 0 impossible (span needs rx) */
...
static void test_wilson(void)
{
     ...
     * Compare in int64: (uint32_t)(flo*1e6)-60 underflows when flo == 0. */
...
int main(void)
{
    ...
    if (failures == 0)
    {
        printf("test_bench_stats: ALL PASS\n");
        return 0;
    }
