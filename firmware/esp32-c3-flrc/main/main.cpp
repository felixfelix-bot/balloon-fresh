...
static void run_tx_continuous() {
    ...
    // CLEAR_ERRORS — prevents PA_OCP_OVP accumulation (RP2040 fix)
    uint8_t cmd_clr_err[] = { 0x01, 0x11, 0x00, 0x00 };
    // DIO_IRQ_CONFIG re-set — TX_DONE bit 19 → DIO9 (RP2040 fix)
    uint8_t cmd_irqcfg[]  = { 0x01, 0x15, 0x09, 0x00, 0x08, 0x00, 0x00 };
    ...
    for (uint32_t i = 0; ; i++) {  // INFINITE — no TX_PKT_COUNT limit
        ...
        // 4. Write TX FIFO (0x0002)
        rf_write_tx_fifo(pkt, FLRC_PKT_SIZE);

        // 5. Set TX (0x020D)
        rf_write_cmd(cmd_set_tx, 5);

        // 6. Wait for TX_DONE — IRQ pin goes HIGH
        uint32_t timeout = 500000;
        while (!irq_high() && --timeout) {}
