...
# ─── udev chmod enforcement ──────────────────────────────────────────────
# Board serial number fragments (matched against udev ID_SERIAL_SHORT).
# When a track acquires a board lock, the matching /dev/ttyACMx is chmod'd
# to 000 — mechanically blocking raw serial.Serial() and pio upload bypass.
# On release, chmod'd back to 666.
BOARD_SERIALS = {
    "tx": "F242D",   # RP2040 TX board (E663B035977F242D)
    "rx": "8332",    # RP2040 RX board
}


def find_ports_for_board(resource: str) -> list:
    """Find /dev/ttyACMx ports matching a board by udev serial number.

    Boards move between ACM0/1/2/3 on reconnect, so we resolve dynamically
    via udevadm rather than hardcoding port numbers.
    """
    import subprocess
    import glob
        ...
        try:
            ...
            for line in r.stdout.split("\n"):
                if "ID_SERIAL_SHORT=" in line:
                    ...
                    if serial_fragment in sn:
                        ports.append(port_path)
                    break  # Only check first ID_SERIAL_SHORT
        except Exception:
            pass
    return ports


def restrict_port_access(resource: str):
    """chmod serial ports to 000 — blocks ALL non-root access.

    Called after acquiring the flock. The lock holder must use the
    BoardSerial wrapper (or `grant` subcommand) to temporarily restore
    access before opening the port.
    """
    ports = find_ports_for_board(resource)
    if not ports:
        return
    for port in ports:
        try:
            os.chmod(port, 0o000)
            print(f"  CHMOD 000: {port} ({resource})", file=sys.stderr)
        except PermissionError:
            print(f"  WARNING: no permission to chmod {port} — "
                  f"run lock script with appropriate privileges", file=sys.stderr)
        except OSError as e:
            print(f"  WARNING: could not chmod {port}: {e}", file=sys.stderr)


def restore_port_access(resource: str):
    """chmod serial ports back to 666 — restores access for all users.

    Called before releasing the flock.
    """
    ports = find_ports_for_board(resource)
    if not ports:
        return
    for port in ports:
        try:
            os.chmod(port, 0o666)
            print(f"  CHMOD 666: {port} ({resource})", file=sys.stderr)
        except PermissionError:
            print(f"  WARNING: no permission to chmod {port}", file=sys.stderr)
        except OSError as e:
            print(f"  WARNING: could not chmod {port}: {e}", file=sys.stderr)
...
def acquire(resource: str, purpose: str, timeout_s: int) -> int:
    ...
    while True:
        ...
        if all_ok:
            ...
            # ─── udev chmod enforcement ───────────────────────────────
            # chmod serial ports to 000 so raw serial.Serial() / pio upload
            # cannot bypass the flock. Lock holder uses BoardSerial or the
            # `grant` subcommand to temporarily restore access.
            for r in resources:
                if r in BOARD_SERIALS:
                    restrict_port_access(r)
...
def release(resource: str, force: bool, steal: bool) -> int:
    ...
    for r in resources:
        ...
        # ─── udev chmod enforcement ───────────────────────────────────
        # Restore port access BEFORE killing sentinel so the board is
        # available immediately after release.
        if r in BOARD_SERIALS:
            restore_port_access(r)
        if sentinel_pid:
            try:
...
def grant(resource: str) -> int:
    """Temporarily restore port access (chmod 666) for the lock holder.

    Called by BoardSerial wrapper before opening a serial connection.
    Only works if your track currently holds the lock for this resource.
    """
    resources = _get_resources(resource)
    identity = _identity()
    granted_any = False
    ...
    for r in resources:
        if r not in BOARD_SERIALS:
            continue
        path = _lock_path(r)
        data = _read_metadata(path)
        if not data:
            print(f"FREE: {r} (no lock held — no restriction in place)", file=sys.stderr)
            continue
        holder = data.get("track", "?")
        if holder != identity["track"]:
            print(f"REFUSED: {r} held by track={holder}, not {identity['track']}",
                  file=sys.stderr)
            continue
        restore_port_access(r)
        granted_any = True

    return 0 if granted_any else 1


def revoke(resource: str) -> int:
    """Re-restrict port access (chmod 000) after the lock holder is done.

    Called by BoardSerial wrapper after opening a serial connection.
    The already-open fd remains usable even after chmod 000.
    """
    resources = _get_resources(resource)
    identity = _identity()
    revoked_any = False
    ...
    for r in resources:
        ...
        holder = data.get("track", "?")
        if holder != identity["track"]:
            # Don't restrict someone else's board
            continue
        restrict_port_access(r)
        revoked_any = True

    return 0 if revoked_any else 1
...
udev chmod enforcement:
  %(prog)s acquire tx     # chmod /dev/ttyACMx → 000 (blocks raw access)
  %(prog)s grant tx       # temporarily chmod → 666 (lock holder only)
  %(prog)s revoke tx      # re-restrict → 000 (after opening serial fd)
  %(prog)s release tx     # chmod → 666 (restore on release)
        """)
    parser.add_argument("action", choices=["acquire", "release", "status", "grant", "revoke"],
    ...
    if args.action in ("grant", "revoke"):
        if not args.resource:
            print("Error: resource (tx/rx/both) required for grant/revoke", file=sys.stderr)
            sys.exit(2)
        if args.action == "grant":
            sys.exit(grant(args.resource))
        else:
            sys.exit(revoke(args.resource))
